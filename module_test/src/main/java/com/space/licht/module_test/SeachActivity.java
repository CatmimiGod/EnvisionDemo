package com.space.licht.module_test;
...
import com.space.licht.lib_seach.adapter.SGAdapter;
import com.space.licht.lib_seach.adapter.SearchAdapter;
import com.space.licht.lib_seach.bean.SanGuoBean;
import com.space.licht.lib_seach.bean.SearchBean;
import com.space.licht.lib_seach.widget.SearchView;
...
public class SeachActivity extends AppCompatActivity {
     ...
     * 通用搜索框
     */
    private SearchView<SearchBean> mCommolySearchView;
    /**
     * 数据显示listview
     */
     ...
     * 三国通用搜索框
     */
    private SearchView<SanGuoBean> mSGCommolySearchView;
    /**
     * 三国数据源
     */
...
//        initView();
    ...
    private void initView() {
        mCommolySearchView = (SearchView) findViewById(R.id.csv_show);
        ...
        // 设置筛选数据
        mCommolySearchView.setSearchDataListener(new SearchView.SearchDatas<SearchBean>() {
        ...
        mListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                Toast.makeText(SeachActivity.this, "当前点击的是" + mCommolySearchView.getFilterDatas().get(i).getSearchName(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void initViewI() {
        mSGCommolySearchView = (SearchView) findViewById(R.id.csv_show);
        mListView = (ListView) findViewById(R.id.lv_show);
        ...
        // 设置筛选数据
        mSGCommolySearchView.setSearchDataListener(new SearchView.SearchDatas<SanGuoBean>() {
            @Override
            public List<SanGuoBean> filterDatas(List<SanGuoBean> datas, List<SanGuoBean> filterdatas, String inputstr) {
                for (int i = 0; i < datas.size(); i++) {
                    // 筛选条件
                    if ((datas.get(i).getSgDescribe()).contains(inputstr) || datas.get(i).getSgName().contains(inputstr)
                            || datas.get(i).getSgPetName().contains(inputstr)) {
                        filterdatas.add(datas.get(i));
                    }
                }
                return filterdatas;
        ...
        mListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                Toast.makeText(SeachActivity.this, mSGCommolySearchView.getFilterDatas().get(i).getSgName() + "字" + mSGCommolySearchView.getFilterDatas().get(i).getSgPetName() + "\n" + mSGCommolySearchView.getFilterDatas().get(i).getSgDescribe(), Toast.LENGTH_SHORT).show();
            }
        });
